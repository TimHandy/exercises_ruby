...
DataMapper::setup(:default, "sqlite3://#{Dir.pwd}/recall.db") # locates in root level, and creates the db if not already present

class Note
  include DataMapper::Resource
  property :id, Serial # keyword Serial makes this unique auto-incrementing
...
put '/:id' do
  ...
  n.updated_at = Time.now
  if n.save
    redirect '/', :notice => "Note successfully updated."
  else
